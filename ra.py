...
def handle_update(u):
    ...
    # /start - SHOW DIFFERENT COMMANDS BASED ON USER/ADMIN
    if lower_text == "/start":
        user_commands = (
            "👋 Welcome!\n\n"
        ...
        admin_commands = (
            ...
            "• /unapprove <user_id>\n"
            "• /approvedlist\n"
            "• /block <device_id> - or reply to message\n"
            "• /unblock <device_id>\n"
            "• /blockedlist - show blocked devices\n"
            "• /stopall - stop all users' monitoring\n"
        )
        
        if is_owner(chat_id):
            send_msg(chat_id, user_commands + admin_commands)
        else:
            send_msg(chat_id, user_commands)
        return
    ...
    if lower_text == "/ping":
        ...
        status_text = (
            ...
            f"📵 Blocked devices: <code>{blocked_count}</code>\n"
        )
    ...
    if lower_text.startswith("/block"):
        ...
        if reply_msg and reply_msg.get("text"):
            # Extract device ID from replied message
    ...
    # Fallback help - SHOW DIFFERENT COMMANDS BASED ON USER/ADMIN
    user_help = (
    ...
    if is_owner(chat_id):
        send_msg(chat_id, user_help + admin_help)
    else:
        send_msg(chat_id, user_help)
